...
ui <- fluidPage(
  titlePanel("Priority Bird Sightings - Connecticut (Last 30 Days)"),
...
server <- function(input, output, session) {
  # Get recent sightings - reactive to allow refresh
  ...
  output$freq_table <- render_gt({
    priority_summary() %>%
      gt() %>%
      tab_header(title = "Bird Sightings in the Last 30 Days") %>%
      cols_label(
  ...
  output$sightings_map <- renderLeaflet({
    leaflet() %>%
      ...
      addCircleMarkers(data = priority_sf(),
                       radius = 6,
                       fillColor = ~pal()(as.numeric(obsDt)),
                       fillOpacity = 0.8,
                       stroke = FALSE,
